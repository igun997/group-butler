...
// mediaStore is the Mongo surface the media path needs: recording the outcome of
// one attachment on the message it belongs to. *messageStore satisfies it as-is.
...
// absent on purpose: it is incremented by saveMedia, not overwritten per attempt.
func mediaFields(m Media) bson.D {
	return bson.D{
		{Key: "media.status", Value: string(m.Status)},
		{Key: "media.kind", Value: string(m.Kind)},
		...
		{Key: "media.durationSeconds", Value: m.DurationSec},
...
// saveMedia records one media outcome on the message it belongs to. `attempts`
// is incremented rather than set, so the record says how many times the worker
// tried rather than only what the last try concluded (§6.3.5).
